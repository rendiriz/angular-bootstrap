...
export class ChartPieComponent implements OnChanges, OnDestroy {
  ...
  constructor(
    ...
    private chartService: ChartPieService
  ) {
    setTimeout(() => {
      this.initData();
    }, 500);
  ...
  initDataChart(): void {
    ...
    this.legends = [
      ...this.dataset.map((content: any, key: any) => {
        return {
          ...
          fillType: 'solid',
          show: true,
          selector: '.pie-' + _.snakeCase(content.text),
          ...content
        };
      })
    ];
    ...
    this.tooltip = d3.select('body').append('div')
      .attr('class', 'chart-tooltip pie-tooltip')
      .style('opacity', 0);
  }
  ...
  drawPie(): void {
    ...
    const pie = arc.selectAll('.pie')
      ...
      .enter()
      .append('g')
      .attr('class', (d: any) => 'pie pie-' + _.snakeCase(d.data.text));
  ...
  drawLegend(): void {
    const legendSize = 20;
    const legendContainer = d3.select('.legend-pie');
    legendContainer.selectAll('.legend-item').remove();

    d3.select('#btn-pie-more').text('Tampilkan lebih banyak');
  ...
  toggleLegend(e: any): void {
    d3.selectAll('.legend-pie .legend-more')
    ...
    if (d3.select('.legend-pie .legend-more').classed('d-none')) {
